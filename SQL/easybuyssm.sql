...
CREATE TABLE `category` (
  ...
  `parentId` int(11) NOT NULL COMMENT '父级分类的id',
  `type` int(11) DEFAULT NULL COMMENT '1一级分类，2二级分类，3三级分类',
  PRIMARY KEY (`id`)
...
CREATE TABLE `product` (
  ...
  `categoryLevel3` int(11) DEFAULT NULL,
  `fileName` varchar(100) NOT NULL COMMENT '上传的文件名',
  `isDelete` int(11) DEFAULT NULL COMMENT '1为删除，0为未删除',
  `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
) ENGINE=InnoDB DEFAULT CHARSET=utf8;
...
CREATE TABLE `user` (
  ...
  `type` int(1) DEFAULT '0' COMMENT '2为超级管理员，1为商家，0为用户',
  `createTime` timestamp NOT NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
  PRIMARY KEY (`id`)
